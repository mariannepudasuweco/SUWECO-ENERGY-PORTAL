...
export function Sidebar({ 
  ...
  const allItems = [
    ...
    { id: 'project' as ModuleType, label: 'Task', icon: <Briefcase size={20} /> },
    { id: 'reports' as ModuleType, label: 'Reports', icon: <FileText size={20} /> },
  ...
  const handleNavigateToNotification = (moduleId: ModuleType, pageId: AppPage, projectId: string) => {
    if (projectId) {
    ...
    // Set view first to get transition setup
  ...
  return (
    <aside className="w-[100px] h-[calc(100vh-32px)] my-4 ml-4 rounded-xl flex-shrink-0 flex flex-col items-center py-6 bg-[#1A1C20] z-50 overflow-y-visible no-scrollbar relative">
      ...
      <div 
        ...
        onClick={() => { setActiveModule(null); setActivePage('dashboard'); }}
      >
        ...
        <span className="text-[9px] text-white/90 font-bold tracking-wider uppercase text-center w-full px-2">STEC Portal</span>
      </div>

      {/* Utility Icons */}
      <div className="flex flex-col gap-4 mb-6 text-[#6B7280] w-full items-center pl-1 pr-1">
        <NotificationCenter onNavigateToNotification={handleNavigateToNotification} />
        <button className="hover:text-white transition-colors" title="Toggle Theme" onClick={() => setTheme(prev => prev === 'light' ? 'dark' : 'light')}>
          {theme === 'light' ? <Moon size={20} strokeWidth={2} /> : <Sun size={20} strokeWidth={2} />}
        </button>
        <button className="hover:text-white transition-colors" title="Home" onClick={() => window.location.href = '/'}>
      ...
      <div className="flex flex-col gap-2 w-full flex-1 overflow-y-auto no-scrollbar">
        {items.map(item => {
          ...
          return (
            <div key={item.id} className="relative w-full flex justify-center group flex-shrink-0">
              ...
              >
                ...
                <span className={`text-[10px] font-bold tracking-wide ${isActive ? 'text-white' : 'text-[#6B7280]'}`}>
                  {item.label}
                </span>
              </button>
            </div>
